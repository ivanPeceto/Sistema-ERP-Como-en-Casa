...
export const getMetodosCobro = async (): Promise<MetodoCobro[]> => {
  const response = await metodoCobroAPIClient.get<MetodoCobro[]>('/api/cobros/metodos/listar/');
  return response.data;
};

/**
 * @brief Crea un nuevo método de cobro (POST a /api/cobros/metodos/crear/).
...
export const createMetodoCobro = async (metodoData: Omit<MetodoCobro, 'id'>): Promise<MetodoCobro> => {
  const response = await metodoCobroAPIClient.post<MetodoCobro>('/api/cobros/metodos/crear/', metodoData);
  return response.data;
};

/**
 * @brief Actualiza un método de cobro existente (PUT a /api/cobros/metodos/editar/?id=...).
...
export const updateMetodoCobro = async (id: number, metodoData: Omit<MetodoCobro, 'id'>): Promise<any> => {
  const response = await metodoCobroAPIClient.put(`/api/cobros/metodos/editar/?id=${id}`, metodoData);
  return response.data;
};

/**
 * @brief Elimina un método de cobro existente (POST a /api/cobros/metodos/eliminar/?id=...).
...
export const deleteMetodoCobro = async (id: number): Promise<any> => {
  ...
  const response = await metodoCobroAPIClient.post(`/api/cobros/metodos/eliminar/?id=${id}`);
  return response.data;
};